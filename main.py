...
def extract_triples(text):
    ...
    for sent in doc.sents:
        entities = [ent for ent in sent.ents if ent.label_ in ["PERSON", "ORG", "GPE", "PRODUCT"]]
        ...
        for e1 in entities:
            for e2 in entities:
                ...
                for token in sent:
                    if token.pos_ == "VERB":
                        # Check if verb is syntactically related to both entities
                        if (e1.start <= token.i <= e1.end) or (e2.start <= token.i <= e2.end):
                            continue
                        verb = token.lemma_
                        break
                if verb:
                    triples.append((e1.text, verb, e2.text))
    return triples

# Extract Triples
all_triples = []
for text in df["content"].dropna().head(200):  # Process first 200 articles for speed
